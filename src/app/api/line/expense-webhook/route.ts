...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify LINE signature (optional in development)
      ...
      if (transaction) {
        ...
        const result = await pool.query(
          ...
          [
            ...
            `Auto-imported from LINE BK (User: ${userId})`,
          ]
        );
...
// Reply function to confirm transaction saved
...
export async function GET() {
  return NextResponse.json({ status: "LINE Expense Webhook is running" });
}
